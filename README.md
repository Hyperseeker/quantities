...
Parse quantities expressed in multiple units:

```javascript
Qty("1 m 20 cm"); // => 1.2 m
Qty("2 kg 500 g"); // => 2.5 kg
Qty("5 ft 11 in"); // => 5.916666666666667 ft
```

Pairs are treated additively and converted to the first pair's unit. All pairs must have compatible units (same dimension). See [Multi-Pair Parsing](docs/multi-pair-parsing.md) for details.
...
The Quantity class provides Math-like static and instance methods for common mathematical operations:

#### Static Methods

Find minimum or maximum from multiple quantities:

```javascript
Quantity.min(Quantity("5 m"), Quantity("10 m"), Quantity("3 m")); // => 3 m
Quantity.min("200 cm", "3 m", "150 cm"); // => 150 cm

Quantity.max(Quantity("5 m"), Quantity("10 m"), Quantity("3 m")); // => 10 m
Quantity.max("100 cm", "2 m"); // => 2 m

// Units must be compatible
Quantity.min("5 m", "10 s"); // throws QuantityError
```
...
```javascript
// Absolute value - returns new Quantity
Quantity("-5 m").abs(); // => 5 m
Quantity("-10 tempC").abs(); // => 10 tempC

// Sign - returns number (-1, 0, or 1)
Quantity("5 m").sign(); // => 1
Quantity("-5 m").sign(); // => -1
Quantity("0 m").sign(); // => 0

// Method chaining works
Quantity("-5 m").abs().to("cm"); // => 500 cm
```
